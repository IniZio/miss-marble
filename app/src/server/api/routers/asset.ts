...
export const assetRouter = createTRPCRouter({
  presignUpload: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const { data: presignedUpload } = await getSupabase().storage.from(PUBLIC_STORAGE_BUCKET_NAME).createSignedUploadUrl(filename);
      const { data: { publicUrl } } = getSupabase().storage.from(PUBLIC_STORAGE_BUCKET_NAME).getPublicUrl(filename);
